...
public record ReviewOneReviewDTO(long id, String title, String commentary, String code, String test, int likes, UserDTO author, Date date, List<CommentDTO> comments) {
    public static ReviewOneReviewDTO from(Review review) {
        ...
        return new ReviewOneReviewDTO(review.getId(), review.getTitle(), review.getCommentary(), review.getCode(), review.getTest(), review.getLikes(),
                UserDTO.from(review.getAuthor()), review.getDate(), comments);
    }
}